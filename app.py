...
client = pymongo.MongoClient(
    "mongodb+srv://mongoadmin:BmmfKb1UkIFbRFl5@cluster0.ybns4.azure.mongodb.net/admin?retryWrites=true&w=majority")
...
def get_species_stats(jsonified=True):
    ...
    for species in Species_Master:
        ...
        species_count_list.append(
            {"species": species, "individual": individuals, "images": image_count, "females": females, "males": males,
             "unknown": unknowns})
...
def index(sitetype="user"):
    ...
    trained_artifact_count = colartifacts.count_documents(
        {"References.s3_image_name": {"$exists": 1}, "MachineLearning.MLType": "Train"})
    ...
    last_model_refresh = colmodelsummaries.find_one({}, projection=["TimeStamp"], sort=[("TimeStamp", -1)]).get(
        "TimeStamp", "")
            ...
            for i in range(num_detections):
                ...
                if USE_DETECTION_CLASSES:
                    ...
                    if detected_species_prediction != "":
                        ...
                        detected_species_confidence = min(100, round(
                            float(artifact["Footprint_Detection"][i].get("confidence", 0)) * FIELD_FACTOR, 2))
                        Detected_Individual_Inference = artifact["Footprint_Detection"][i].get(
                        ...
                        best_spec, best_ind = UpdateBestPredictions(best_spec, best_ind,
                                                                    {"value": detected_species_prediction,
                                                                     "confidence": detected_species_confidence},
                                                                    Detected_Individual_Inference)
    ...
    try:
        ...
        if References != "":
            ...
            if num_detections > 0:
                ...
                for i in range(num_detections):
                    coordinates = artifact["Footprint_Detection"][i]["coordinates"].split(',')
                    shape = [(float(coordinates[0]), float(coordinates[1])),
...
@public_endpoint
@app.route(AzureAuthentication.REDIRECT_PATH)  # Its absolute URL must match your app's redirect_uri set in AAD
def authorized():
    try:
        cache = _load_cache()
